            ...
            <div class="card hover-shadow-lg pb-0">
                <div class="card-header border-0 pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-0 {{ (strtotime($project->end_date) < time()) ? 'text-danger' : '' }}" data-toggle="tooltip" data-original-title="{{__('End Date')}}">{{ Utility::getDateFormated($project->end_date) }}</h6>
                        </div>
                         <div class="text-right">
                            <span class="badge badge-xs badge-{{ (\Auth::user()->checkProject($project->id) == 'Owner') ? 'success' : 'warning'  }}" data-toggle="tooltip" >{{ __(\Auth::user()->checkProject($project->id)) }}</span>
                        </div>
                        <div class="text-right">
                          <span class="clearfix"></span>
                          <span class="badge badge-pill badge-{{\App\Models\Project::$status_color[$project->status]}}">{{ __(\App\Models\Project::$project_status[$project->status]) }}</span>
                        </div>
                    </div>
                </div>
