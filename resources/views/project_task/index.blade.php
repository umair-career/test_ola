...
@section('content')
    <div class="row pb-0 overflow-hidden">
        <div class="col-12 container-kanban">
            <div class="board min-750" data-plugin="dragula" data-containers='{{json_encode($stageClass)}}'>
                @foreach($stages as $stage)
                    <div class="tasks px-0">
                        <div class="m-2">
                            <div class="row">
                                <div class="col-6">
                                    <h6 class="mb-0 mt-0 task-header">{{ $stage->name }}</h6>
                                </div>
                                @can('create project task')
                                    <div class="col-6 text-right">
                                        <div class="actions">
                                            <a class="action-item mr-2" href="#" data-url="{{ route('projects.tasks.create',[$project->id,$stage->id]) }}" data-ajax-popup="true" data-size="lg" data-title="{{__('Add Task in ').$stage->name}}">
                            ...
                            <div class="card-list-body task-list-items" id="task-list-{{ $stage->id }}" data-status="{{$stage->id}}">
                                @foreach($stage->tasks as $taskDetail)
                                    <div class="card card-progress draggable-item border shadow-none pb-0 mb-0" id="{{$taskDetail->id}}" style="{{ !empty($taskDetail->priority_color) ? 'border-left: 2px solid '.$taskDetail->priority_color.' !important' :'' }};">
                                        <div class="card-body">
                                                        ...
                                                        <div class="dropdown action-item">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                @can('view project task')
                                                                    <a href="#" data-url="{{ route('projects.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" class="dropdown-item">{{ __('View') }}</a>
                                                                @endcan
                                            ...
                                            @can('view project task')
                                                <a class="h6 task-name-break" href="#" data-url="{{ route('projects.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true">{{ $taskDetail->name }}</a>
                                            @endcan
                                            <div class="row align-items-center">
                                                <div class="col-12">
                                                    <div class="actions d-inline-block">
                                                        @if(count($taskDetail->taskFiles) > 0)
                                                            <div class="action-item mr-2"><i class="fas fa-paperclip mr-2"></i>{{ count($taskDetail->taskFiles) }}</div>
                                                        @endif

                                                        @if(count($taskDetail->comments) > 0)
                                                            <div class="action-item mr-2"><i class="fas fa-comment-alt mr-2"></i>{{ count($taskDetail->comments) }}</div>
                                                        @endif

                                                        @if($taskDetail->checklist->count() > 0)
                                                            <div class="action-item mr-2"><i class="fas fa-tasks mr-2"></i>{{ $taskDetail->countTaskChecklist() }}</div>
                                                        @endif
                                                    </div>
                                                </div>
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var html = "<div class='list-group-item px-0'>" +
                                ...
                                "                                <img src='" + data.user.profile + "' alt='" + data.user.name + "' class='avatar-sm rounded-circle'>" +
                                "                            </a>" +
                                "                        </div>" +
                                "                        <div class='col ml-n2'>" +
